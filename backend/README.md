...
## Storage & Streaming Architecture (Phase 3, Phase 4, Phase 5, Phase 6, Phase 7, Phase 8, Phase 9, Phase 10, Phase 11 & Phase 12)
...
> [!IMPORTANT]
> **Privacy Invariant**: Notification preferences control future notification creation (`userFollowed`, `voiceNoteLiked`). `GET /api/notifications/preferences` returns **ONLY** preferences for `req.user._id`. Disabling preferences suppresses notification creation but **DOES NOT** delete historical ActivityEvents or existing notifications.
...
### 7. In-App Notifications & User Controls (Phase 11 & Phase 12)
...
- **`NotificationPreference`** (`src/models/NotificationPreference.js`): User notification controls (`userId`, `userFollowed`, `voiceNoteLiked`, `createdAt`, `updatedAt`). Unique index on `{ userId: 1 }`. Default booleans are `true`.

---

## Current Status & Phase 12 Scope

> [!NOTE]
> **Phase 0 through Phase 12 Status: COMPLETE.**
> Notification preference model (`userFollowed`, `voiceNoteLiked`), lazy default creation, preference endpoints (`GET /api/notifications/preferences`, `PATCH /api/notifications/preferences`), preference-aware notification suppression, ActivityEvent preservation, user isolation, and regression safety are fully implemented and verified via 48 automated tests (487 total test cases across all phases).
...
1. **Phase 13 — User Comments & Discussions:** VoiceNote comments, comment replies, and comment notifications.
