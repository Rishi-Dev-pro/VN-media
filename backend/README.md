...
## Storage & Streaming Architecture (Phase 3, Phase 4, Phase 5, Phase 6, Phase 7, Phase 8, Phase 9, Phase 10 & Phase 11)
...
User Controller / Follow Controller / Voice Note Controller / Album Controller / Like Controller / Activity Controller / Notification Controller
        ↓
   User Service / Follow Service / VoiceNote Service / ActivityEvent Service / Notification Service
        ↓
...
> [!IMPORTANT]
> **Privacy Invariant**: Notifications are in-app delivery records created from ActivityEvents (`USER_FOLLOWED`, `VOICE_NOTE_LIKED`). `GET /api/notifications` returns **ONLY** notifications where `recipientId = req.user._id`. Notifications do **NOT** grant access to stream or download private VoiceNotes or private Albums, and do **NOT** expose credentials (`email`, `passwordHash`).
...
| `PATCH /api/notifications/read-all` | Any | Authenticated User | ALLOW (Mark all recipient's notifications read) | `200 OK` |
| `PATCH /api/notifications/:id/read` | Any | Recipient | ALLOW (Mark single notification read) | `200 OK` |
...
### 7. In-App Notifications (Phase 11)
...
## Data Models (Phase 1, Phase 6, Phase 7, Phase 8, Phase 10 & Phase 11)
...
- **`Notification`** (`src/models/Notification.js`): In-app notifications (`recipientId`, `actorId`, `type`, `targetType`, `targetId`, `activityEventId`, `metadata`, `readAt`, `createdAt`). Unique index on `{ activityEventId: 1 }`, lookup indexes on `{ recipientId: 1, createdAt: -1 }` and `{ recipientId: 1, readAt: 1 }`.

---

## Current Status & Phase 11 Scope

> [!NOTE]
> **Phase 0 through Phase 11 Status: COMPLETE.**
> Persistent in-app notifications (`GET /api/notifications`, `PATCH /api/notifications/:id/read`, `PATCH /api/notifications/read-all`), ActivityEvent-driven consumer logic (`USER_FOLLOWED`, `VOICE_NOTE_LIKED`), unique `activityEventId` idempotency, unread counts, self-notification exclusion, actor sanitization, recipient isolation, and regression safety are fully implemented and verified via 71 automated tests (439 total test cases across all phases).
...
1. **Phase 12 — User Comments & Discussions:** VoiceNote comments, comment replies, and comment notifications.
